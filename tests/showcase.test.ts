...
test('should collect links', async () => {
	...
	expect(showcase_1?.links).toMatchObject(
		author_1_links.map((link) => ({ url: link, type: 'unknown' }))
	);

	expect(showcase_2?.author).toBe(author_3);
	expect(showcase_2?.links).toHaveLength(1);
	expect(showcase_2?.links).toMatchObject(
		author_3_links.map((link) => ({ url: link, type: 'unknown' }))
	);
});
...
test('should sanitize GitHub repo links', async () => {
	...
	expect(
		showcases.at(0)?.links.every((link) => link.url.match(/^https:\/\/github\.com\/\w+\/repo$/))
	).toBe(true);
});
...
test('should collect links from the same user spread across multiple comments', async () => {
	...
	expect(showcases.at(0)?.links).toMatchObject(
		[...author_comment_1_links, ...author_comment_2_links].map((link) => ({
			url: link,
			type: 'unknown',
		}))
	);
...
describe('GitHub repo link languages', () => {
	test('should collect languages', async () => {
		...
		function expectLanguagesToMatch(
			languages: ShowcaseGitHubRepoLink['languages'],
			linkLanguages: TestRepoLanguage[]
		) {
		...
		assert(
			showcase_link_2?.type === 'github_repo',
			'Expected the second link to be a GitHub repo.'
		);
		...
		const scraper = getTestScrapper([
			{
				...
				links: [
					{
						languages: [{ node: { name: faker.lorem.word() }, size: getTestRepoLanguageSize() }],
						url: link,
					},
				],
			},
