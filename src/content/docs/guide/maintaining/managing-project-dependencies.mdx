...
{/* prettier-ignore */}
<p class="lead">**Every dependency is code you ship without having written it.** Choose them carefully, lock their versions, update them in small steps, and audit them regularly. That keeps the project stable and secure as it grows.</p>
...
A big part of managing dependencies is choosing them well from the start. Look at four things:

1. **Reputation and Popularity**: Choose dependencies that are well-maintained with regular updates, widely used, and backed by good documentation and an active community. This ensures that issues are addressed promptly and that the dependency is reliable.
...
Dependency management tools make it easier to track, install, and update the dependencies your project requires. Each ecosystem has its own:
...
Most of them have features for locking versions, auditing security, and simplifying updates—features that can significantly ease dependency maintenance over time.
...
Most dependency management tools provide mechanisms for pinning dependencies. For example, npm writes a `package-lock.json`, Yarn a `yarn.lock` and pnpm a `pnpm-lock.yaml`: commit it. In Python, `Pipfile.lock` or `poetry.lock` serves a similar purpose.
...
2. **Test Before Upgrading**: Even minor updates can introduce unexpected changes. Before deploying updated dependencies, ensure your testing suite covers the main features. If you don't have tests, consider creating them for critical areas before updating.
...
Dependencies can introduce security risks, so secure them:

1. **Regularly Audit Dependencies**: Use tools like `npm audit`, `pip-audit`, `pipenv check`, or other command-line tools to scan your dependencies for known vulnerabilities. Make regular audits a habit to catch security risks early.
